...
server.listen(PORT, async () => {
  ...
  try {
    ...
    const topicRes = await fetch(`http://localhost:${PORT}/topics`, {
      method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ name: "Notifications" }),
    }).then((r) => r.json());
    topicId = topicRes.topic.id;

    // Create subscribers
    const s1 = await fetch(`http://localhost:${PORT}/subscribers`, {
      method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ name: "UserA" }),
    }).then((r) => r.json());
    sub1Id = s1.subscriber.id;
    const s2 = await fetch(`http://localhost:${PORT}/subscribers`, {method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ name: "UserB" }),
    }).then((r) => r.json());
    sub2Id = s2.subscriber.id;

    // Subscribe UserA
    await fetch(`http://localhost:${PORT}/subscribers/subscribe`, {
      method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ topicid: topicId, subscriberid: sub1Id }),
    });

    // Publish first event
    await fetch(`http://localhost:${PORT}/events/publish`, {
      method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ topicid: topicId, message: "Hello World" }),
    });
    // Subscribe UserB (late subscriber)
    await fetch(`http://localhost:${PORT}/subscribers/subscribe`, {
      method: "POST",headers: { "Content-Type": "application/json" },body: JSON.stringify({ topicid: topicId, subscriberid: sub2Id }),
    });

    // Publish second event
    await fetch(`http://localhost:${PORT}/events/publish`, {
      method: "POST",headers:{ "Content-Type": "application/json" },body: JSON.stringify({ topicid: topicId, message: "Second Event" }),
    });
